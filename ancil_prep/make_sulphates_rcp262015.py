...
def constant_2015():
  ...
  """
     Constant SO2 emissions (2014 values) from 2015 onwards
  """
...
def make_ancil_const():

  #Include final name that is short enough for CPDN filename restrictions
  final_anc = 'SO2DMS_const2014'
  cmip6_data_dir = '/data/data/TCRE_data/n48_cmip6_hist/'
  nc_name = 'SO2DMS_const2014.nc'
  ...
  #Cut to make the 10-year segments for distribution
  for i in range(0,len(first_year)):
    cut_nc_name = 'SO2DMS_const2014_'+str(first_year[i])+'_'+str(last_year[i])+'.nc'
    cut_anc_name = 'SO2DMS_const2014_'+str(first_year[i])+'_'+str(last_year[i])
    os.system('ncks -d time,'+str(first_time[i])+','+str(last_time[i])+' '+cmip6_data_dir+'SO2DMS_const2014.nc'+' '+cmip6_data_dir+cut_nc_name)
