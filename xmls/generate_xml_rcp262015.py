...
def xmlgen():
  ...
  """ Generation of xml for TCRE experiment with RCP2.6-2015 forcing between 2014 and 2024"""

  #Start year of the model integrations 
  model_start_year=2014

  #File containing XML template
  templatefile='./headers/main_site_header_hadcm3.xml'
  #Output XML filename
  outxmlfile='./hadcm3s_tcre_rcp262015_'+time.strftime("%d%m%Y")+'.xml'
  ...
  f_restart_umids = open('../../cpdn_anal/batch_data/batch_514.txt','r')
  old_umids = list(set([line[:4] for line in f_restart_umids]))

  #Load the lookup dictionary for the parameter umids
  f_param_umids = open('./dicts/umiddict_tcre_main_2000.txt','r')
  param_umids = {f[5:9]:f[:4] for f in f_param_umids}

  #Set xml tags that are the same for all workunits
  file_sulphox='sulpc_oxidants_19_A2_1990'
  file_ozone='ozone_rcp262015_2014_2025'
  file_solar='solar_cmip6_rescale_f'
  file_ghg='ghg_rcp262015_20142025'
  file_flux = 'waterfix.ancil.be.32'
  file_spec_sw = 'spec3a_sw_3_asol2c_hadcm3'
  file_spec_lw = 'spec3a_lw_3_asol2c_hadcm3'
  file_volc = 'NAT_VOLC'
  file_volcanic = 'latvolc_zeros_20142025'
  run_years='10'
  ...
  files_so2dms=['SO2DMS_rcp262015_2014_2025','SO2DMS_const2014_2014_2025']
  ...
  outtreeroot.find('batch_tech_info').text = 'TCRE main experiments'
  ...
  for old_umid in old_umids:
    #Loop over the two different SO2 futures
    for file_so2dms in files_so2dms:

       param_data = param_dataset[param_dataset['umid']=='y'+param_umids[old_umid][1:]]
       ...
       for theta in theta_list:
        try:
           ...
           SubElement(Parameters, 'file_atmos').text= old_umid +'_2014.astart'
           SubElement(Parameters, 'file_ocean').text= old_umid + '_2014.ostart'
